/**
 * Static mirror of prisma/seed.ts, for UI pages to render before NextAuth/API
 * wiring lands (build steps 6 & 18). Field names/shapes intentionally match
 * the Prisma schema so swapping these for real queries later is a drop-in.
 */
...
export type TaskType = "LEARN" | "BUILD" | "DSA" | "REVIEW" | "MOCK" | "READ";
...
export interface CurriculumResource {
  title: string;
  url: string;
  type: "BOOK" | "COURSE" | "VIDEO" | "ARTICLE" | "TOOL" | "REPO";
  priority: number;
}
...
export const PHASES: Phase[] = [
  {
    number: 1,
    name: "Python Backend",
    description: "Move from tutorial-level scripts to production-grade Python services.",
    color: "backend",
  },
  {
    number: 2,
    name: "AI Mastery",
    description: "Master the full LLM application stack — APIs, retrieval, agents, and evals.",
    color: "ai",
  },
  {
    number: 3,
    name: "Cloud + Infra",
    description: "Take your AI stack from a laptop deploy to real cloud infrastructure.",
    color: "cloud",
  },
  {
    number: 4,
    name: "DSA — The Gate",
    description: "Pattern-first data structures & algorithms practice for interviews.",
    color: "dsa",
  },
  {
    number: 5,
    name: "System Design",
    description: "Design large-scale systems, HLD, LLD, and distributed architectures.",
    color: "sysdesign",
  },
  {
    number: 6,
...
export const MONTHS: CurriculumMonth[] = [
  {
    number: 1,
...
/** Task-type badge color, reusing phase hue tokens rather than a parallel set. */
export const TASK_TYPE_COLOR: Record<TaskType, PhaseColorKey> = {
  LEARN: "cloud",
  BUILD: "ai",
  DSA: "dsa",
  REVIEW: "sysdesign",
...
export function getPhase(phaseNumber: number): Phase {
  ...
  if (!phase) throw new Error(`Unknown phase number: ${phaseNumber}`);
  return phase;
}

export function getMonth(monthNumber: number): CurriculumMonth {
  const month = MONTHS.find((m) => m.number === monthNumber);
  if (!month) throw new Error(`Unknown month number: ${monthNumber}`);
  return month;
}
